...
final class TransferViewModel {
    ...
    init(
        ...
        eligibilityService: TransferEligibilityService = TransferEligibilityService() ) {
            self.repository = repository self.eligibilityService = eligibilityService
        } func attemptTransfer( amount: Decimal, from: Account, to: Account ) async {
            /* First check whether the transfer is allowed.*/
            let eligibility = eligibilityService.canTransfer( amount: amount, from: from )
            switch eligibility { case .failure(let error):
                /* Do NOT call the repository if validation fails.*/
                onError?(error) case .success: do {
                    /*Only make the repository/network call if eligible.*/
                    try await repository.transfer( amount: amount,
                                                   from: from,
                                                   to: to )
                    onSuccess?()
                } catch let error as TransferError {
                    onError?(error)
                } catch { /* The current TransferError enum has no generic network-error case, so there is nothing specific we can report here.*/
                }
            }
        }
}
